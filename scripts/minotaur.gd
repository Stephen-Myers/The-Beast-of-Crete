...
@export_range(0.0, 1.0) var wander_random_chance: float = 0.0
...
func _hunt_step() -> void:
	if _cell == _target:
		...
		var checked := hunt_hiding_places_to_check - _hunt_hiding_places.size()
		if checked >= hunt_hiding_places_to_check or _hunt_hiding_places.is_empty():
			_state = State.WANDER
			_pick_new_target()
			return
		_assign_next_hunt_target()
